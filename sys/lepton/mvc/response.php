...
//            fseek($file,0,$seekto);
    ...
    static function redirect($to, $code=302) {
        ...
        if ((substr($to, 0, 7) == 'http://') ||
            ...
            $host = (request::getDomain()); // strlen($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME'];
            if (substr($to, 0, 1) == '/') {
                $location = sprintf('%s://%s%s', $schema, $host, $to);
            } elseif (substr($to, 0, 1) == '.') { // Relative Path
                $location = sprintf('%s://%s/', $schema, $host);
                $pu = parse_url($to);
